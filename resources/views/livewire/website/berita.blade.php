<div class="container px-2 py-3 mx-auto md:px-0">
    <section class="w-full h-[200px] md:h-[600px] overflow-hidden rounded-xl mb-3 md:mb-6 relative shadow-md">
        <img src="{{ asset('storage/image/' . ($danin_menu[4] ? $danin_menu[4]->gambar : 'default.png')) }}"
            alt="" class="object-cover w-full h-full">
    </section>
    <div class="w-full my-12">
        <div class="my-4 text-4xl text-center text-orange-900 ghr">
            @if ($danin_menu[4])
                {{ $danin_menu[4]->informasi }}
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        @empty
            <div class="w-full text-4xl text-center ghr">Nantikan event dari kami</div>
        @endforelse
